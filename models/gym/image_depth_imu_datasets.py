...
class IMU_Image_Depth_Dataset(Dataset):
    def __init__(self, goal_position, load_from_file=False, preprocessed_data_path=None, data_dir=None, image_folder_name=None, depth_folder_name=None):
        self.datasets = []
    ...
    def load_data(self, data_dir, goal_position):
        ...
        for folder in os.listdir(data_dir):
            ...
            if self.depth_folder_name == None:
            ...
            for idx in range(len(images)):
                if images[idx].endswith('.png') and depths[idx].endswith('.png'):
                    ...
                    state = np.hstack((img_embedding,depth_embedding, positions[idx], imu_data_list[idx].reshape(-1)))  # Stack the embeddings and positions horizontally
                    states.append(state)
...
if __name__ == "__main__":
    ...
    dataset = IMU_Image_Depth_Dataset(goal_position=goal_position, load_from_file=True, preprocessed_data_path="./data/preprocessed_data_v0.pkl", data_dir=data_dir, transform=None)
    ...
    trajectories = IMU_Image_Depth_Dataset(goal_position=goal_position, load_from_file=True, preprocessed_data_path="./data/preprocessed_data_v0.pkl", data_dir=data_dir, transform=None)

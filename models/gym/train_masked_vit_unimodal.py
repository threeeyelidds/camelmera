...
import time
import tqdm
from masked_vit import MaskedViTEncoder, MaskedViTDecoder, mae_factory,random_tube_masking
...
masked_encoder = MaskedViTEncoder.from_pretrained(model_name, config=vit_config)
masked_decoder = MaskedViTDecoder.from_pretrained(model_name, config=vit_config)
unimodel = mae_factory(masked_encoder, masked_decoder)
...
while not all(item[1] == 0 for item in env_epoch):
    ...
    def train(model, dataloader, optimizer, scheduler, device):
        ...
        for batch in tqdm.tqdm(dataloader):
            ...
            optimizer.zero_grad()
            masked_pixel_values, mask_indices = random_tube_masking(pixel_values, mask_probability=0.15)
            encoder_outputs = model.encoder(masked_pixel_values)
            decoder_outputs = model.decoder(encoder_outputs, mask_indices)

            loss = torch.nn.functional.mse_loss(decoder_outputs, pixel_values)
            loss.backward()

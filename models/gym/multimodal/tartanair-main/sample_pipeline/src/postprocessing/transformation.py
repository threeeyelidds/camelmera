...
def motion2pose(data, poseinit=None):
    '''
    data: N x 12
    all_pose: (N+1) x 12
    poseinit: 12
    ...
    if poseinit is not None:
        temp = poseinit.reshape(1,12)
        pose[:3, :] = poseinit.reshape(3, 4)
...
def sixdof2SO3(input_6dof):
    if len(input_6dof.shape) == 1:
        input_ = np.expand_dims(input_6dof, axis = 0)
    else:
        input_ = input_6dof
    ...
    R1 = R1 / np.expand_dims(np.linalg.norm(R1, axis=1), 0).T
    R2 = R2 - np.dot(R1,R2.T).diagonal().reshape(-1,1)*R2

    R2 = R2 / np.expand_dims(np.linalg.norm(R2, axis=1), 0).T

    R3 = np.cross(R1, R2)

    R1 = np.expand_dims(R1, 1)  
    R2 = np.expand_dims(R2, 1)  
    R3 = np.expand_dims(R3, 1)

    Rotmat = np.stack([R1,R2,R3], axis=1).squeeze(axis = 2)

    if len(input_6dof.shape) == 1:
        return np.transpose(Rotmat, (0,2,1))[0] #dim = 3,3

    return np.transpose(Rotmat, (0,2,1)) #dim = N, 3,3

def SO2sixdof(input_rotmat):  
    batch_size = input_rotmat.shape[0]
    return np.stack(np.split(input_rotmat[:, 0:3, 0:2],2, axis= 2), axis=1).reshape(batch_size, 6)

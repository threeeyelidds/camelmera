...
def convert_DEA_2_Velodyne_XYZ(d, e, a):
    """
    e and a are defined in the camera frame with z-axis pointing forward, y-axis downwards.
...
class ScanlineParams(object):
    ...
    def find_coordinates(self, E, resA, offA):
        ...
        a    = np.linspace( a0, a1, int( (a1-a0)/resA + 1 ), endpoint=True )
...
class SimulatedLiDAR(object):
    ...
    def initialize(self):
        ...
        #     ax.scatter(sp.x4, sp.y4, s=0.01, alpha=1)
        #     plt.pause(0.1)
        #     plt.draw()
        # plt.savefig('before.png', dpi=800)
    ...
    def extract(self, depthList, maxDist=None, depthChangeMask=None):
        """
        ...
        masks = np.concatenate( depthChangeMask, axis=1 )

        # the opencv remap define (0,0) at the center of the pixel
        # we define (0,0) at the corner of the pixel
        cvquery_x = self.scanlineMaps[0] - 0.5
        cvquery_y = self.scanlineMaps[1] - 0.5

        # Sample depth.
        sdLN = cv2.remap( depth, cvquery_x, cvquery_y, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_WRAP )
        sdNR = cv2.remap( depth, cvquery_x, cvquery_y, interpolation=cv2.INTER_NEAREST, borderMode=cv2.BORDER_WRAP )
        mask = cv2.remap( masks, cvquery_x, cvquery_y, interpolation=cv2.INTER_NEAREST, borderMode=cv2.BORDER_WRAP ) == 1
        # print(np.sum(mask))

        # Sanity check sample pixels
        # xint, yint = np.meshgrid(range(depth.shape[1]), range(depth.shape[0]))
        # src = np.stack((xint, yint), axis=2)
        # dstNR = cv2.remap(src, self.scanlineMaps[0], self.scanlineMaps[1], interpolation=cv2.INTER_NEAREST)
        # plt.ion()
        # fig, ax = plt.subplots(figsize=(36, 12))
        # ax.set_ylim([0,480])
        # ax.set_xlim([0,640 * 4])
        # for i in range(len(dstNR)):
        #     ax.scatter(dstNR[i,:,0], dstNR[i,:,1], s=0.01, alpha=1)
        #     plt.pause(0.1)
        #     plt.draw()
        # plt.savefig('after_NR.png', dpi=800)

        # src = src.astype(np.float32)
        # dstLN = cv2.remap(src, self.scanlineMaps[0], self.scanlineMaps[1], interpolation=cv2.INTER_LINEAR)
        # dstLN = np.round(dstLN).astype(int)
        # plt.ion()
        # fig, ax = plt.subplots(figsize=(36, 12))
        # ax.set_ylim([0,480])
        # ax.set_xlim([0,640 * 4])
        # for i in range(len(dstLN)):
        #     ax.scatter(dstLN[i,:,0], dstLN[i,:,1], s=0.01, alpha=1)
        #     plt.pause(0.1)
        #     plt.draw()
        # plt.savefig('after_LN.png', dpi=800)

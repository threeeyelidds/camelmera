...
set env=CyberpunkExposure
set gamma=1.0
set seglabel=C:\tartanair-v2\Package\CyberpunkExposure\Cyberpunk\Content\buffered_id_map_Showcase.json
...
cd /d %codedir%
@REM start /wait python -m collection.collect_images --environment-dir %outputdir%\%env% --posefile-folder %posedir%\%env%\pose_easy --data-folder Data_easy --cam-list 0_1_2_3_4_5_6_7_8_9_10_11
...
start /wait python -m postprocessing.data_validation --data-root %outputdir% --env-folders %env% --data-folders Data_easy,Data_hard --create-video  --analyze-seg --rgb-depth-filter --np 12 
